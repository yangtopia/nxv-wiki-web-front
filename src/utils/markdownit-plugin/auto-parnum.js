/* eslint-disable */
/*! markdown-it-auto-parnum 2.0.0  @license MIT */
(function(f) {
        ...
        function(require, module, exports) {
          ...
          class Counter {
            ...
            insertAt(i, heading = false) {
              ...
              if (this.options.addLinks)
                token.attrPush(['href', `#${sign}${this.value}`]);
          ...
          module.exports = function plugin(md, options = {}) {
            function autoParNum(state) {
              ...
              let skippedElements = (
                options.skippedElements || 'footnote_block'
              )
                .replace(/(.+?)(,|$)+/g, '$1_open,$1_close$2')
                .split(',')
                .filter((v) => v);
              ...
              let numberHeadings =
                options.numberHeadings === false ? false : true;
              ...
              options.headingLevels =
                typeof options.headingLevels === 'undefined'
                  ? 1
                  : options.headingLevels;
              ...
              options.addLinks =
                typeof options.addLinks !== 'undefined'
                  ? Boolean(options.addLinks)
                  : true;

              let showUnNestedParnum = !!options.showUnNestedParnum;
              ...
              for (let i = 0; i < state.tokens.length; i++) {
                ...
                if (
                  ...
                  (numberedElements.indexOf(state.tokens[i].tag) > -1 &&
                    /_open$/.test(state.tokens[i].type)) ||
                  numberedElements.indexOf(state.tokens[i].type) > -1
                ) {
                  tag = /^h\d$/.test(state.tokens[i].tag)
                    ? state.tokens[i].tag
                    : 'p';
                  if (
                    ...
                    if (!scheme[tag]) {
                      scheme[tag] = setNum;
                      if (scheme.p.length < scheme[tag].length)
              ...
              if (num.elements.length <= num.headingLevels) {
                ['h1', 'h2', 'h3', 'h4', 'h5', 'h6'].some((tag) => {
                  if (
                    headingCount[tag] > (showUnNestedParnum ? 0 : 1) &&
              ...
              for (let i = 0; i < state.tokens.length; ++i) {
                if (
                  state.tokens[i].type === 'paragraph_number' ||
                  (i > 0 && state.tokens[i - 1].type === 'paragraph_number')
                )
                  continue;
                ...
                if (
                  nesting === 0 &&
                  ((numberedElements.indexOf(token.tag) > -1 &&
                    /_open$/.test(state.tokens[i].type)) ||
                  ...
                  if (
                    state.tokens[i + 2].tag === state.tokens[i].tag &&
                  ...
                  if (
                    ...
                    (i === 0 ||
                      !(
                        ...
                        ['auto', 'on', 'start'].indexOf(
                          state.tokens[i - 1].attrGet(sign),
                        ) > -1
                      ))
                  )
                    continue;
                ...
                // Tags that may affect numbering
                  ...
                  if (numberHeadings && num.elements.indexOf(token.tag) > 0)
                    num.insertAt(i, true);
                }
            ...
            md.renderer.rules.paragraph_number = function(
              tokens,
              idx,
              options,
              env,
              ...
              return `<a${slf.renderAttrs(tokens[idx])}>${
                tokens[idx].content
              }</a>`;
            };
